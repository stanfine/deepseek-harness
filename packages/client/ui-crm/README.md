...
Flexible semantic results use a separate persisted `meta.crmAnalysis` version. The Client accepts only matching request and result metadata with declared metric and dimension ids, finite or null metric values, bounded rows and warnings, coherent completeness facts, and allowlisted drilldown ids. It rejects unknown row keys, duplicate columns, invalid dates and unexpected persisted fields before presentation. Chart selection reads these validated values without recalculating business metrics: summaries use KPI cards, trends use lines, rankings use horizontal bars, comparisons use bars, complete nonnegative compositions use donuts, and compatible two-metric results use bar-line charts. Dense, multi-dimensional or incompatible results retain a table fallback, and nulls remain chart gaps.
