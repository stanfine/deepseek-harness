...
Flexible semantic results use a separate persisted `meta.crmAnalysis` version. The Client validates closed column semantics, coverage and completeness before rendering. The final retained `{ content, meta }` projection, including escaped tool text and outer field names, is measured against the lower of the deployment response budget and the shipped 1 MiB UTF-8 publication ceiling. Trends retain every calendar bucket accepted by the planner budget; ranked and composition results remain bounded by Top N. A donut requires one additive metric, one mutually exclusive dimension, complete nonnegative values and at most eight categories. Ratio, distinct-count, overlapping, high-cardinality or incomplete compositions use bars or tables.
...
Marketing workflow cards validate persisted recommendation, campaign-plan, inactive-draft, status, and aggregate-result metadata before rendering. They show recommendation priority, action and limitations; plan readiness and estimated audience size; and opaque MA audience and campaign ids after creation. Draft cards accept only `inactive` status and never expose approval, start, delivery, coupon-issue, edit, or delete controls. Rejected or customer-bearing metadata falls back to the raw tool result.
...
The `crm_analyze` and `crm_drilldown` keyed presenters apply the `meta.crmAnalysis` validator before rendering. They show summary rows as KPI cards and grouped rows as the deterministic chart selected from the validated intent, columns, completeness, and comparison metadata. Every chart has a named source table containing current, comparison, change, null, and partial values; unavailable reasons remain beside the affected table values. Current and comparison record counts, coverage windows, earliest observed source date, metric descriptions, limitations, warnings, and both date windows remain adjacent to the evidence. Rejected metadata keeps the generic raw result. On narrow screens, KPI cards collapse to one column, definitions stack, charts shorten, and tables scroll without hiding their labels.
