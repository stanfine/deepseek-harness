...
Flexible semantic results use a separate persisted `meta.crmAnalysis` version. The Client validates closed column semantics, coverage and completeness before rendering. The final retained `{ content, meta }` projection, including escaped tool text and outer field names, is measured against the shipped 1 MiB UTF-8 publication budget. Trends retain every calendar bucket accepted by the planner budget; ranked and composition results remain bounded by Top N. A donut requires one additive metric, one mutually exclusive dimension, complete nonnegative values and at most eight categories. Ratio, distinct-count, overlapping, high-cardinality or incomplete compositions use bars or tables.
