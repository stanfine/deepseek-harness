...
interface TestAnalysis {
  crmAnalysis: {
    ...
    data: {
      ...
      request: TestRequest
      columns: {
        dimensions: Array<{ id: string; name: string; dataType: string }>
        metrics: Array<{ id: string; name: string; format: string; description: string; limitations: string[] }>
      }
...
export function analysis(): TestAnalysis {
  ...
  const data = { version: 1, request,
    columns: { dimensions: [{ id: 'channel', name: '渠道', dataType: 'keyword' }],
      metrics: [{ id: 'sales_amount', name: '销售额', format: 'currency', description: '成交金额', limitations: [] }] },
    ...
    completeness: { complete: true, missingDimensionDocuments: 0, omittedDocuments: 0, limitedRows: 0,
      countErrorUpperBound: 0, approximateMetrics: [] },
