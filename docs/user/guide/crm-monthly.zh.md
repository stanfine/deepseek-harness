...
初始目录包含销售额、订单数、件数、购买人数、客单价、连带率、件单价、购买频次和人均消费。日、周、月、渠道、子渠道、门店、门店类型和订单类型由拥有这些概念的数据源提供；产品系列和 SKU 属于商品行数据源。结果记录当前值与对比值、不可用原因、覆盖范围、遗漏分组、计数误差和允许的后续维度。客户端验证持久化结果，并确定性选择指标卡、折线图、横向条形图、对比柱状图、环图或表格。模型不能提供源字段、Elasticsearch DSL、公式或 ECharts 选项。
