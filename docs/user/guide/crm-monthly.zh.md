...
输入：`分析 2025 年 3 月相对上月的营销机会，先给建议，不要创建活动。` Agent 从 CRM 汇总证据评估受治理的机会，并返回最多三条排序建议。它会调用 `crm_activation_catalog` 查询当前 MA 的分组、分类、内容以及有效 CDP 标签，不自行编造外部 id。准备计划时，它把选中的目标标签、排除标签、分组、分类和内容 id 传给 `crm_campaign_plan`。当前工程再次查询实时目录，拒绝不存在或停用的选择，把校验后的名称和 id 写入计划摘要，并返回汇总人群估算、动作、指标、限制和就绪原因，全程不执行写入。
