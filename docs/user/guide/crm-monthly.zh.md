...
输入：`分析 2025 年 3 月相对上月的营销机会，先给建议，不要创建活动。` Agent 从 CRM 汇总证据评估受治理的机会，并返回最多三条排序建议。它调用 `crm_activation_catalog` 查询当前 MA 的分组、分类和内容，不自行编造外部 id。准备计划时，它把选中的分组、分类和内容 id 传给 `crm_campaign_plan`。当前工程根据已记录证据和部署方策略构造新的 MA 人群，再次查询实时 MA 目录，拒绝不存在或停用的触达选项，把解析后的值写入计划摘要，并返回汇总人群估算、动作、指标、限制和就绪原因，全程不执行写入。CDP 标签是否可用不会阻止计划生成。

示例中的渠道机会读取可覆盖周报周期所使用的商品行数据源，人群估算也使用同一数据集的购买人数指标。MA 分组、分类和内容分别限制返回数量，分组较多时不会遮蔽生成画布所需的内容。
