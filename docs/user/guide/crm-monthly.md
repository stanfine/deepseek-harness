...
1. Complete the repository setup in [Development](../../development.md) and configure a DeepSeek model in the application. Run the following commands from the repository root in zsh. Use an Elasticsearch account restricted to reading the two configured indices; the plugin is not a substitute for database permissions.
