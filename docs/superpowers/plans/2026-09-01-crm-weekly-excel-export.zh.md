...
**参考：** [Agent Note](../../../.agents/notes/implemented/feature/2026-09-01-crm-weekly-excel-export.zh.md)

1. 先为工作簿模型投影、公式保护和标识排除编写失败测试；实现使用配置 artifact-tool 导入的 `weekly-workbook.ts`。
2. 先为导出根目录限制、随机 id、过期和清理编写失败测试；实现 `excel-exports.ts`。
3. 在 `crm-tools.ts` 中增加 `crm_export_weekly_excel`，增加由 Host 持有的 `crm-excel-host.ts` provider 和 `/api/crm.export` 路由，并在预设配置中增加明确的渲染器和导出设置。
