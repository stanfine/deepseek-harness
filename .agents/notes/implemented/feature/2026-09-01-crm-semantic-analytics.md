...
Source sums publish a `missing` companion and become unavailable when matching documents omit the configured field; ratios propagate that state. Both comparison modes require observed history at the configured-time-zone comparison start. Date filters compile to local-day half-open ranges, every date grouping is budgeted before access, and trends preserve chronological buckets without Top-N slicing. Persisted columns carry closed metric additivity and dimension composition semantics, so donuts require an additive metric and mutually exclusive categories. Tool text and persisted metadata share one 1 MiB UTF-8 publication budget.
