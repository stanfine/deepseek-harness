...
`crm_campaign_create_draft` accepts only a current-session plan id and the literal `create_inactive_draft`. A scoped `tools/pre-execute` listener asks for host approval at that write and delegates every read. The coordinator revalidates audience count and canvas prediction, records progress events, uses a deterministic business key, and reconciles ambiguous writes before retrying. It creates an audience and an inactive campaign only. No exposed service or tool approves, starts, sends, issues benefits, edits, or deletes.
